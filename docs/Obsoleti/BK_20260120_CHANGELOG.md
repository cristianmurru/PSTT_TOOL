...
## [1.1.0-alpha.1] - 2026-01-18

### Added - Kafka Integration (STEP 1-4/8)

#### STEP 1: Foundation ✅
- 🚀 **Kafka Integration Foundation** - Iniziata integrazione Kafka per pubblicazione messaggi su topic
  - Dipendenze: `kafka-python-ng==2.2.2` e `aiokafka==0.10.0` aggiunte a requirements.txt
  - Modelli Pydantic completi per configurazione Kafka (`app/models/kafka.py`):
    - `KafkaConnectionConfig`: gestione connessione con supporto PLAINTEXT/SSL/SASL
    - `KafkaProducerConfig`: configurazione producer (compression, batch, idempotence)
    - `KafkaExportConfig`: configurazione export schedulato verso Kafka
    - `KafkaMetrics`: metriche pubblicazione (latenza, throughput, success rate)
    - `BatchResult`: risultato invio batch messaggi
    - `KafkaHealthStatus`: status health check producer
  - Estensione `app/core/config.py`: 36 nuove variabili configurazione Kafka
  - Helper `get_kafka_config()` per caricamento configurazione completa
  - Supporto multi-cluster Kafka in `connections.json` (sezione `kafka_connections`)
  - File `.env.example` documentato con tutte le variabili Kafka e esempi configurazione
  - Test suite: 25 test passed per validazione modelli e configurazione

#### STEP 2: KafkaService Base ✅
- 📡 **KafkaService Implementation** - Producer Kafka completo e funzionante (`app/services/kafka_service.py`)
  - **Connection Management**: Connessione asincrona con supporto security (PLAINTEXT/SSL/SASL)
  - **Message Publishing**: Invio messaggi singoli con retry automatico
  - **JSON Serialization**: Encoder custom per `datetime`, `date`, `Decimal`, `None`
  - **Health Check**: Verifica connettività broker con latenza e broker count
  - **Metrics Tracking**: Tracciamento automatico latenza, throughput, success rate
  - **Error Handling**: Gestione completa errori Kafka (timeout, broker unavailable, generic)
  - **Logging Strutturato**: Log dettagliati per troubleshooting con livelli appropriati
  - **Context Manager**: Supporto `async with` per gestione automatica risorse
  - **Connection Pooling**: Riutilizzo connessioni producer per performance
- Test suite completa: 24 test passed con mock Kafka producer
  - Test serializzazione JSON custom (datetime, Decimal, dict complessi)
  - Test connessione (success, no brokers, errori generici)
  - Test invio messaggi (success, timeout, broker unavailable, headers)
  - Test health check (connesso, disconnesso, riconnessione automatica)
  - Test metriche (tracking, calcolo success rate, reset)
  - Test lifecycle (close, context manager)

#### STEP 3: Batch Publishing & Performance ✅
- ⚡ **Batch Publishing Optimization** - Pubblicazione batch ad alte performance (`app/services/kafka_service.py`)
  - **`send_batch()`**: Invio batch messaggi con chunking intelligente
    - Chunking automatico per batch grandi (configurable chunk_size, default 100)
    - Processing parallelo messaggi all'interno di ogni chunk
    - Flush periodico buffer ogni 10 chunk per ottimizzare memoria
    - Auto-reconnect se producer disconnesso
    - Progress logging dettagliato per batch grandi
    - Calcolo throughput automatico (msg/sec)
    - Tracking errori con limite 100 errori salvati (evita memory bloat)
  - **`send_batch_with_retry()`**: Retry automatico con backoff esponenziale
    - Retry automatico fino a `max_retries` tentativi (default: 3)
    - Success rate threshold configurabile (default: 95%)
    - Backoff esponenziale: 100ms → 200ms → 400ms (configurabile)
    - Retry intelligente: continua se success rate < 95%
    - Gestione eccezioni con logging dettagliato
  - **`_chunk_messages()`**: Helper per suddivisione batch in chunk ottimali
- Test suite batch publishing: 16 test passed
  - Test batch vuoto, singolo messaggio, batch piccoli (10 msg), batch grandi (1000 msg)
  - Test con headers custom, partial failure handling
  - Test auto-reconnect e connection failure
  - Test retry: success first attempt, success after failures, all attempts fail
  - Test exception handling durante retry
  - Test chunking logic, metrics update, success rate calculation
  - **Performance test**: Throughput > 100 msg/sec verificato con 1000 messaggi

#### STEP 4: SchedulerService Integration ✅
- 🔗 **Scheduler Kafka Integration** - Export automatico risultati query verso Kafka
  - **Estensione `SchedulingItem` model** (`app/models/scheduling.py`):
    - Aggiunto `SharingMode.KAFKA` enum value
    - Nuovi campi: `kafka_topic`, `kafka_key_field`, `kafka_batch_size`, `kafka_include_metadata`, `kafka_connection`
    - Validazione batch_size range 1-10000
  - **Estensione `SchedulingHistoryItem` model**:
    - Campi tracking Kafka: `kafka_topic`, `kafka_messages_sent`, `kafka_messages_failed`, `kafka_duration_sec`
    - Campo `export_mode`: 'filesystem', 'email', 'kafka' per distinguere tipo export
  - **Metodo `_execute_kafka_export()`** in `SchedulerService` (`app/services/scheduler_service.py`):
    - Caricamento configurazione Kafka da `connections.json`
    - Trasformazione risultati query in messaggi Kafka (key/value)
    - Aggiunta metadata automatica: source_query, source_connection, export_timestamp, export_id
    - Invio batch con `send_batch_with_retry()` (max 3 retry)
    - Logging dettagliato: throughput, success rate, durata
    - Tracking completo in `scheduler_history.json`
  - **Pipeline export Kafka** in `run_scheduled_query()`:
    - Supporto `sharing_mode='kafka'` accanto a filesystem/email
    - Esecuzione query → trasformazione dati → export Kafka
    - Error handling robusto con fallback su history
    - Tracking `export_mode` in tutte le esecuzioni
- Test suite integrazione: 15 test passed
  - Test configurazione SchedulingItem con campi Kafka
  - Test validazione batch_size e enum SharingMode
  - Test `_execute_kafka_export()`: success, no topic, connection not found, partial failure
  - Test metadata inclusion/exclusion nei messaggi
  - Test tracking export_mode in history (kafka/filesystem/email)
  - Test SchedulingHistoryItem con campi Kafka

### Technical Details
- **Security**: Supporto PLAINTEXT, SSL, SASL_PLAINTEXT, SASL_SSL con meccanismi PLAIN/SCRAM
- **Performance**: 
  - Configurazione ottimizzata (Snappy compression, batching, idempotence)
  - Chunking intelligente per gestione batch grandi (20K-200K msg/giorno)
  - Processing parallelo messaggi con `asyncio.to_thread`
  - Flush periodico per evitare memory bloat
  - Throughput target: >100 msg/sec (verificato con test)
- **Scheduler Integration**:
  - Export Kafka completamente integrato nel sistema scheduling esistente
  - Supporto multi-cluster Kafka via connections.json
  - Metadata automatici per traceability
  - Retry automatico con success rate threshold 95%
  - Zero breaking changes su funzionalità esistenti
- **Validation**: Pydantic validators per bootstrap servers, topic names, acks mode
- **Reliability**: Retry automatico, backoff esponenziale, success rate monitoring
- **Feature Flag**: `KAFKA_ENABLED` per abilitare/disabilitare funzionalità
- **Async/Await**: Architettura completamente asincrona compatibile con FastAPI
- **Resilience**: Retry automatico, fallback connessione, graceful degradation

### Test Coverage
- **Total Tests**: 49 (25 config + 24 service)
- **Success Rate**: 100% (49/49 passed)
- **Coverage**: > 90% per componenti Kafka

### Next Steps
- STEP 3: Batch publishing e ottimizzazione performance (send_batch, chunking, compressione)
- STEP 4: Integrazione con `SchedulerService` per job automatici
- STEP 5: API endpoints e UI dashboard Kafka
