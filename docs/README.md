...
- 🆕 **Integrazione Kafka**: Publishing asincrono batch e singolo con retry automatico, metriche e multi-cluster support
...
5. **Accedi all'interfaccia**:
   ...
   - Kafka Dashboard: http://localhost:8000/kafka (se configurato)

## 📁 Struttura Progetto
```
PSTT_Tool/
├── app/                        # Codice applicazione
│   ├── models/                 # Modelli dati (scheduling, kafka, etc.)
│   ├── services/               # Logica business (scheduler, kafka, metrics)
│   ├── api/                    # Endpoints REST API (queries, scheduler, kafka)
│   ├── templates/              # Template HTML (dashboard, kafka_dashboard)
...
## 🔧 Configurazione Database e Kafka
### connections.json

Il file `connections.json` contiene la configurazione delle connessioni database e Kafka:
```json
{
...
**Note sulla configurazione Kafka**:
- La sezione `kafka_connections` è **opzionale** - se non presente, l'app funziona normalmente senza funzionalità Kafka
- Supporto multi-cluster per gestire ambienti dev/prod/staging
- Security protocols: `PLAINTEXT`, `SSL`, `SASL_PLAINTEXT`, `SASL_SSL`
- SASL mechanisms: `PLAIN`, `SCRAM-SHA-256`, `SCRAM-SHA-512`
- Password tramite variabili ambiente con sintassi `${VAR_NAME}`
### File .env
...
# Password per cluster Kafka production
KAFKA_PROD_PASSWORD=your_secure_kafka_password

# Configurazione avanzata Kafka (valori di default già configurati)
...
PSTT Tool supporta la pubblicazione di risultati query verso topic Kafka, permettendo l'integrazione con pipeline di data streaming e sistemi event-driven. L'integrazione è completamente **opzionale** e non impatta le funzionalità esistenti.

### Caratteristiche Principali

- **Multi-cluster support**: Gestione di più cluster Kafka (dev/prod/staging)
- **Publishing asincrono**: Invio batch e singolo con retry automatico
- **Security completa**: Supporto SSL/TLS e SASL (PLAIN, SCRAM-SHA-256/512)
- **Metriche real-time**: Dashboard con throughput, latency, success rate
- **Performance**: >100 msg/sec con batch da 1000 messaggi
- **Retry automatico**: Backoff esponenziale (100ms → 200ms → 400ms)
- **Zero breaking changes**: Completamente retrocompatibile

### Configurazione

Aggiungi la sezione `kafka_connections` in `connections.json` (vedi esempio sopra). Le schedulazioni possono utilizzare la modalità "Kafka" per pubblicare risultati su topic:

**Campi schedulazione Kafka**:
- `kafka_topic`: Nome topic destinazione
- `kafka_key_field`: Campo da usare come chiave messaggio (opzionale)
- `kafka_batch_size`: Dimensione batch per invio (default: 100)
- `kafka_include_metadata`: Include metadati source_query, timestamp, etc.

### Dashboard Kafka

Accedi a `/kafka` per:
- **Overview metriche** last 24h (messaggi inviati, success rate, latency avg/p90/p99)
- **Grafici real-time** throughput e latency con Chart.js
...
- `GET /api/kafka/connections` - Lista connessioni Kafka configurate
...
1. **Configurare connessione** in `connections.json` (sezione `kafka_connections`)
2. **Creare schedulazione** con modalità "Kafka"
3. **Specificare topic** e configurazione batch
4. **Monitorare** da dashboard `/kafka`

### Metriche e Monitoring

Le metriche vengono salvate in `exports/kafka_metrics.json` e includono:
- Throughput (msg/sec)
- Latency (avg, p50, p90, p99)
- Success/failure rate
- Breakdown per topic e connessione
- Cleanup automatico >30 giorni

### Performance Tuning

Configura in `.env` per ottimizzare:

```bash
KAFKA_BATCH_SIZE=100           # Messaggi per batch
KAFKA_COMPRESSION_TYPE=snappy  # Compressione (none|gzip|snappy|lz4|zstd)
KAFKA_ACKS=all                 # Acknowledgment (-1=all|0=none|1=leader)
KAFKA_TIMEOUT_MS=30000         # Timeout connessione
```

### Documentazione Completa

Per setup dettagliato, troubleshooting e best practices:
- **Setup Guide**: [docs/KAFKA_SETUP.md](docs/KAFKA_SETUP.md)
- **Operational Runbook**: [docs/KAFKA_RUNBOOK.md](docs/KAFKA_RUNBOOK.md)
- **Configuration Examples**: [docs/KAFKA_CONFIGURATION_EXAMPLES.md](docs/KAFKA_CONFIGURATION_EXAMPLES.md)

### Tool di Benchmark

Usa `tools/kafka_benchmark.py` per testare performance:

```bash
# Test batch 1000 messaggi
...
- `POST /api/scheduler/scheduling` - Crea schedulazione
- `PUT /api/scheduler/scheduling/{idx}` - Aggiorna schedulazione
- `DELETE /api/scheduler/scheduling/{idx}` - Elimina schedulazione

**Kafka** (nuovo):
- `GET /api/kafka/connections` - Lista connessioni Kafka
- `POST /api/kafka/test` - Test connessione broker
- `POST /api/kafka/send` - Invio messaggio singolo
- `POST /api/kafka/batch` - Invio batch messaggi
- `GET /api/kafka/metrics/*` - Metriche e monitoring

**Monitoring**:
- `GET /api/monitoring/health` - Health check
- `GET /api/system/restart` - Riavvio applicazione
...
**Test Coverage**:
- Kafka: 111 test (config, service, metrics, API, scheduler integration)
- Scheduler: 45 test
- API: 40 test  
- Overall: 76% coverage
...
### v1.1.0 (2026-01-21) - Integrazione Kafka
- ✅ **Kafka Integration completa**: Publishing asincrono batch/singolo con retry automatico
- ✅ **Multi-cluster support**: Gestione multipli cluster Kafka via connections.json
- ✅ **Dashboard Kafka**: UI real-time con metriche throughput/latency/success rate
- ✅ **Security completa**: Supporto SSL/TLS e SASL (PLAIN, SCRAM-SHA-256/512)
- ✅ **API Kafka**: 8 nuovi endpoints per test, publishing, metrics
- ✅ **Scheduler integration**: Export schedulazioni verso topic Kafka
- ✅ **Metrics service**: Tracking persistente con aggregazione multi-dimensionale
- ✅ **Benchmark tool**: Utility per test performance (tools/kafka_benchmark.py)
- ✅ **Documentazione completa**: 3 nuovi documenti (SETUP, RUNBOOK, EXAMPLES)
- ✅ **111 nuovi test**: Coverage Kafka al 76%
- ✅ **Zero breaking changes**: Completamente retrocompatibile
